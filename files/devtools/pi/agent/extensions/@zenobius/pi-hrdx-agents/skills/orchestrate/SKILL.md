...
Call only the parent-facing `hrdx_workflow` tool for workflow execution:

```ts
hrdx_workflow({ action: "prepare", path: ".pi/plans/<run>/workflow.js" })
```
...
```ts
hrdx_workflow({ action: "start", runId: "<run from packet>" })
```
...
```ts
hrdx_workflow({ action: "cancel", runId: "<run>" })
```
...
Hrdx, children, and installed packages retain the user's system permissions.
