...
export type TurnState =
  ...
  | { kind: "active"; startedAt: number; source: "activity" | "hrdx" | "fallback"; activity?: ActivityDetail }
...
export type PaneObservation =
  | { kind: "unknown" }
  | { kind: "present"; observedAt: number; agentStatus: HrdxAgentStatus }
...
export interface SubagentLifecycle {
  ...
  /** Latest optional Pi detail, independent of Hrdx coarse turn state. */
...
export function observePaneInspection(
  ...
  // A local interrupt has higher precedence than coarse Hrdx status. Hrdx
...
export function observeActivity(
  ...
  // Hrdx owns coarse turn state. Pi detail may enrich an authoritative
  // Hrdx-working turn, or provide a fallback only while pane status is unknown.
...
export function projectLifecycle(lifecycle: SubagentLifecycle, now: number): LifecycleProjection {
  ...
  // Pi activity is optional enrichment. Only authoritative Hrdx inspection
  ...
  switch (turn.kind) {
    ...
    case "active": {
      ...
      return { kind: "active", label: turn.source === "hrdx" ? "agent working" : "agent active", stateDurationSince: turn.startedAt };
    }
    case "blocked":
