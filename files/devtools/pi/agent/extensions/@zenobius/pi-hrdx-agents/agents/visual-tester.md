...
**What is needed:** Host project must provide Chrome remote debugging and a CDP helper at `scripts/cdp.mjs`. Pi Hrdx Agents does not ship these.
```
...
Visual QA is read-only with respect to application source and does not need an isolated Git worktree. If the target app is running from a retained worker worktree, test that checkout in place but do not switch its branch, commit, integrate, or remove its Hrdx workspace. Write only the requested report artifact.
