...
hrdx is a terminal multiplexer built for AI agent workflows — like tmux, but each pane knows if it holds a real PTY session for Codex CLI, Claude Code, pi, zot, a custom harness, or a plain shell. This repo manages hrdx config under `files/shells/hrdx/` and symlinks pieces of it into the real hrdx config directory via mise.
...
- This repo's managed files: `files/shells/hrdx/themes/*.json`, `files/shells/hrdx/keys.json`, and `files/shells/hrdx/harness.json`, each symlinked from `mise.unix.toml` (`~/.config/hrdx/...`) and `mise.windows.toml` (`%AppData%\hrdx\...`). Examples in this repo: `files/shells/hrdx/themes/rose-pine-moon.json`, `files/shells/hrdx/harness.json` (registers `lazygit` and `nvim` as quick-launch harnesses, no `busy` detection needed for either).
- `sounds.json` is managed by mise. It is symlinked from `files/shells/hrdx/sounds.json` in `mise.unix.toml` and `mise.windows.toml`.
