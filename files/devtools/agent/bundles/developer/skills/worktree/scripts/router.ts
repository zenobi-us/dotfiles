...
const sessionContextCmd = app
  ...
  .run(() => {
    ...
    console.log(
      JSON.stringify(
        `\n\n<worktree-session muxer="${muxer}" agent="${agent}">\nDetected once at session start by the \`worktree\` skill's Rule 1/Rule 2 logic (\`detectMuxer\`/\`detectAgent\` in scripts/router.ts). Use these values directly instead of running \`scripts/router.ts detect-muxer\`/\`detect-agent\` again. zot has no self-identifying signal and always reports as \`unknown-agent\` — pass an explicit \`--agent zot\` override to \`route\` when you know better. If the muxer or agent changes mid-session (for example the operator attaches a new terminal), re-run the detect commands instead of trusting this stale value.\n</worktree-session>`,
      ),
    );
  });
