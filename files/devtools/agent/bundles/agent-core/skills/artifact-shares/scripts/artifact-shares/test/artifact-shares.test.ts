...
import { appendType, assertShareName, hashDir, readKinds, readTypes, slugify, yamlString } from "../lib";
...
test("the CLI runs from its shebang and lists its commands", async () => {
  ...
  for (const command of ["create", "share", "list", "sync", "doctor", "check", "self-test"]) {
  ...
  expect(output).toContain("checks:types");
