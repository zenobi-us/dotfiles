...
Use the bundled `scripts/private-share.mjs` CLI as the only mutation path. The published `gh-pages` branch MUST obey [the branch contract](files/assets/web/llms.txt): shares live under `s/<hash>/`, directory archives live at `s/<hash>.zip`, and each share has one `sessions.jsonl` record.
