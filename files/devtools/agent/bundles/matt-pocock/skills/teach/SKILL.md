...
- `./files/assets/*`: Reusable **components** shared across lessons. See [Assets](#assets).
...
Lessons are built from reusable **components**, stored in `./files/assets/`: stylesheets, quiz widgets, simulators, diagram helpers, and anything else a second lesson could reuse.

Reuse is the default, not the exception. Before authoring a lesson, read `./files/assets/` and build from the components already there. When a lesson needs something new and reusable, write it as a component in `./files/assets/` and link to it; never inline code a future lesson would duplicate.
