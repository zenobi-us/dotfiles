...
Reach for it once the work is written down as a spec or split into tickets and you're ready to turn that into code. If the spec doesn't exist yet, write it first — for that, use skill to-spec, or use skill to-tickets to break a spec into tickets. If you just want to build something test-first without a full spec, drop to use skill tdd directly.

## Pre-agreed seams

The idea `implement` runs on is the **seam** — the stable interface a feature is tested at, chosen before any code is written. It doesn't invent seams mid-build; it uses the ones already picked (during use skill to-spec) and writes tests against them via use skill tdd. Working at pre-agreed seams is what keeps the implementation honest: the tests target something durable, so the code underneath can move without the tests moving.
...
Reach for it after the work has been specced and sequenced, not before. Its key neighbours: use skill to-tickets, which produces the tickets — each declaring its blocking edges — that it works through, and use skill tdd, which it drives internally to write the tests at each seam before running its own use skill code-review pass and committing. When you're unsure which skill or flow fits, use skill ask-matt routes you.
