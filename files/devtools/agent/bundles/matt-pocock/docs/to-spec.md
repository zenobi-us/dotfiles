...
Reach for it once a change has been talked through and the domain language is settled, and you want that shared understanding written down before any code is written. If you *haven't* aligned yet, grill first — for that, use skill grill-with-docs. To split the finished spec into tickets, use skill to-tickets.

## Prerequisites

`to-spec` publishes into your issue tracker, so use skill setup-matt-pocock-skills must have configured the tracker and triage labels for this repo first. It applies the `ready-for-agent` label itself — no separate triage pass needed.
...
Reach for it after the plan and domain language are resolved, and before you break the work into implementation tickets. Its key neighbours: use skill grill-with-docs, which sharpens the context so the spec is precise, and use skill to-tickets, which turns the spec into a set of tickets for use skill implement to build. When you're unsure which skill or flow fits, use skill ask-matt routes you.
