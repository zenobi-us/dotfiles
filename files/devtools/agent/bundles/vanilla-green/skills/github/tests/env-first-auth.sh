...
rm -f "$TMP_ROOT/op.calls"
(cd "$TMP_ROOT/repo" && PATH="$TMP_ROOT/bin:$PATH" STUB_KEYRING_OK=1 GH_TOKEN=op://vault/github/user "$REPO_ROOT/skills/github/scripts/files/commands/label-add.sh" 42 test-label >/dev/null)
assert_eq "$(wc -l <"$TMP_ROOT/op.calls")" "1" "label-add falls back to keyring for unresolved GH_TOKEN"

rm -f "$TMP_ROOT/op.calls"
(cd "$TMP_ROOT/repo" && PATH="$TMP_ROOT/bin:$PATH" STUB_KEYRING_OK=1 GITHUB_TOKEN=op://vault/github/user "$REPO_ROOT/skills/github/scripts/files/commands/label-remove.sh" 42 test-label >/dev/null)
...
ENVEOF
rm -f "$TMP_ROOT/op.calls"
output=$(cd "$TMP_ROOT/repo" && PATH="$TMP_ROOT/bin:$PATH" "$REPO_ROOT/skills/github/scripts/files/commands/label-add.sh" 42 test-label)
...
rm -f "$TMP_ROOT/op.calls"
output=$(cd "$TMP_ROOT/repo" && PATH="$TMP_ROOT/bin:$PATH" "$REPO_ROOT/skills/github/scripts/files/commands/label-remove.sh" 42 test-label)
