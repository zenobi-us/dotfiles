...
PR_REVIEW_STATUS="$TEST_DIR/../scripts/files/commands/pr-review-status.sh"
...
cp "$PR_REVIEW_STATUS" "$PR_STATUS_ROOT/files/commands/pr-review-status.sh"
cp "$FIXTURES/pr-review-status/github-api.sh" "$PR_STATUS_ROOT/lib/github-api.sh"
cp "$FIXTURES/pr-review-status/sticky-comment.sh" "$PR_STATUS_ROOT/files/commands/sticky-comment.sh"
cp "$FIXTURES/pr-review-status/pr-threads.sh" "$PR_STATUS_ROOT/files/commands/pr-threads.sh"
chmod +x "$PR_STATUS_ROOT/files/commands/pr-review-status.sh"
chmod +x "$PR_STATUS_ROOT/files/commands/sticky-comment.sh"
chmod +x "$PR_STATUS_ROOT/files/commands/pr-threads.sh"
...
status_out=$(BOT_REVIEWERS=' pending-bot , pending-bot-extra , approved-live , unknown-bot , approved-bot ' \
    ...
    "$PR_STATUS_ROOT/files/commands/pr-review-status.sh" 42)
