...
Run from the main checkout of a git repo with an `origin` remote. New-work claims require authenticated `gh` access and `flock` for fail-closed PR discovery and per-issue serialization. Optionally add committed settings in `vstack.settings.toml` and local files/secrets/overrides in `.env.local`.
