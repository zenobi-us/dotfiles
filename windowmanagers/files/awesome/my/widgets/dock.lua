...
local function add_category(menu, main_category)
    ...
    menu:add_widget(radical.widgets.separator(menu,
        (menu.position == "left" or menu.position == "right") and radical.widgets.separator.HORIZONTAL or
        radical.widgets.separator.VERTICAL))
    --     else
    --         offset = 0
    --     end
...
local function load_item(menu, content, path)
    ...
    local item          = menu:add_item { icon = content.Icon, tooltip = content.Name,
        button2 = function(_i, _m, mods, geo)
...
local function load_from_dir(menu)
    fd_async.ini.load_dir(dir, nil, util.getdir("config") .. "/blind/arrow/Icon/"):connect_signal("file::content",
...
local function default_icon_transformation(image, data, item)
    ...
    if (not fg_col) and beautiful.fg_dock_1 and beautiful.fg_dock_2 then
        fg_col = { type = "linear", from = { 0, 0 }, to = { 0, s:get_height() },
            stops = { { 0, beautiful.fg_dock_1 }, { 1, beautiful.fg_dock_2 } } }
    end
