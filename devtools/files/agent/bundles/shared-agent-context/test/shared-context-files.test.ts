...
test("lists files from the active shared-context root", async () => {
  const repositoryRoot = await fs.mkdtemp(path.join(os.tmpdir(), "shared-agent-context-repo-"));
  const sharedBase = await fs.mkdtemp(path.join(os.tmpdir(), "shared-agent-context-shared-"));
