...
description: Delegates structured, multi-source research with verification and confidence levels; use for evidence-based investigations and decision support; produces traceable research files. This workflow is explicit-only because it writes research artifacts.
---

**Explicit-only:** Run this workflow only when the user explicitly requests research and authorises creation of the output files.
