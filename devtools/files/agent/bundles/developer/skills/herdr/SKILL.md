...
Detect the current coding harness with the repository script. Pass its output to Herdr:

```bash
herdr agent start <name> --kind $(scripts/identify-harness.sh) --pane <pane-id> -- <agent-args...>
```

Do not repeat the harness detection logic in a prompt. Stop if the script cannot run.

When a command runs from a non-target worktree space, open the target worktree with `herdr worktree open`. Start the agent in the returned worktree space and root pane.

When a command runs from the target worktree space, create a new tab in the current workspace with `herdr tab create --workspace "$HERDR_WORKSPACE_ID" --cwd <worktree-path> --no-focus`. Start the agent in the returned tab's root pane.

Default to a sibling pane in the current tab and the current working directory for other agent work. Do not create a workspace, tab, worktree, or different cwd unless the user explicitly requests that topology or location.
...
```bash
herdr agent start reviewer --kind $(scripts/identify-harness.sh) --pane <returned-pane-id>
```
...
```bash
herdr agent start reviewer --kind $(scripts/identify-harness.sh) --pane <returned-pane-id> -- <agent-args...>
```
