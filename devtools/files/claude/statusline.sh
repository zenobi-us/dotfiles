...
SHARED_CONTEXT_CLI="$HOME/Projects/dotfiles/devtools/files/agent/bundles/shared-agent-context/cli.ts"
...
if [ -f "$SHARED_CONTEXT_CLI" ]; then
    if shared_context_cache_is_stale; then
        REPORT=$(cd "$DIR" && bun run "$SHARED_CONTEXT_CLI" report --json 2>/dev/null)
