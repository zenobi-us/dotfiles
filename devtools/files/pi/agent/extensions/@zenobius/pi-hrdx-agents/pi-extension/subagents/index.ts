...
import { listHrdxWorktrees } from "./hrdx.ts";
import {
...
const SubagentParams = Type.Object({
	...
	worktree: Type.Optional(
		Type.Object({
			branch: Type.String({
				minLength: 1,
				description: "New branch name for an isolated Hrdx-managed Git worktree",
			}),
			base: Type.Optional(
				Type.String({
					description:
...
const ROLE_PACK_DISCOVERY_EVENT = "pi-hrdx-subagents:roles:discover:v1";
...
function legacyExternalCliDiagnostic(
	...
	return {
		...
		message: `Role "${resolvedAgentName}" requests external CLI "${cli}" in ${path}. pi-hrdx-agents is Pi-only; remove the cli and cli-model fields and select Claude through an authenticated Pi provider/model ID.`,
		path,
...
function muxUnavailableResult() {
	return {
		content: [
			{
				type: "text" as const,
				text: `Subagents require hrdx. ${terminalSetupHint()}`,
			},
		],
		details: { error: "hrdx not available" },
	};
}
...
const BUNDLED_WORKTREE_WARNINGS: Readonly<Record<string, string>> = {
	scout:
		...
		"Hrdx worktree workspaces persist until explicitly removed.",
	reviewer:
...
function formatWorktreeHandoff(worktree: WorktreeHandoff): string {
	...
	lines.push(
		"After review and preservation, close the hrdx workspace, then remove the worktree with:",
...
function ensureLifecycle(running: RunningSubagent): SubagentLifecycle {
	...
	if (
		...
		// evidence was recorded. Hydrate as Hrdx-style waiting and let the
...
function requestSubagentInterrupt(
	...
	try {
		...
		return {
			error:
				`Failed to send Escape to subagent "${running.name}" via hrdx: ` +
...
function buildWorkflowChildCommand(params: {
	...
	const denied =
		"caller_ping,subagent_done,subagent,subagent_interrupt,subagent_resume,subagents_list,hrdx_workflow";
	const env = [
...
/**
 * Launch a subagent: creates the hrdx pane, builds the command, and
...
export default function subagentsExtension(pi: ExtensionAPI) {
	...
	pi.on("session_shutdown", async (event, _ctx) => {
		...
		try {
			...
			// Best effort during parent shutdown; the Hrdx pane remains recoverable.
		}
	});
	...
	const deliverWorkflowOutcome = (
		...
		try {
			selectCompletionApi(pi, runtime.pi).sendMessage(
				{
					customType: "hrdx_workflow_result",
					content,
					display: true,
	...
	if (shouldRegister("subagent"))
		pi.registerTool({
			...
			label: "Subagent",
			description:
				"Spawn a sub-agent in a dedicated terminal hrdx pane, or in an isolated Hrdx-managed Git worktree when worktree is provided. " +
	...
	if (shouldRegister("subagent_resume"))
		pi.registerTool({
			...
			description:
				"Resume a previous Pi-backed sub-agent session in a new hrdx pane. " +
	...
	pi.registerCommand("btw", {
		description:
			"Open an ephemeral side-question session in a background Hrdx tab",
		handler: async (args, ctx) => {
			...
			try {
				...
				ctx.ui.notify("BTW opened in a background Hrdx tab.", "info");
			} catch (error) {
				if (surface) {
					try {
						closePane(surface);
	...
	pi.registerCommand("worktree", {
		...
		handler: async (args, ctx) => {
			...
			if (trimmed === "list") {
				...
				try {
					const worktrees = listHrdxWorktrees(ctx.cwd);
					ctx.ui.notify(
						worktrees
							.map(
								(worktree) =>
