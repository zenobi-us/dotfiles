...
description: >
  ...
  subagents extension running inside hrdx.
---
...
During the session, the planner can spawn **`scout`** when a design decision depends on existing code it hasn't read. These are internal to the planning session. You'll see them in hrdx but don't need to intervene.
