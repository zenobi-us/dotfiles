...
const COMMAND_NAME = "generate-commit-message";
const COMMAND_ALIAS = "gcm";
const CONFIG_PATH = path.join(homedir(), ".pi", "agent", "generate-commit-message.json");
...
function pickCopilotDefaultMode(ctx: ExtensionContext): string {
	...
	const freeLike = available.find((id) => /(mini|flash|nano|haiku|free)/i.test(id));
...
function runGenerateCommit(args: string, ctx: ExtensionContext, pi: ExtensionAPI): void {
	...
	pi.sendUserMessage(`Call the subagent tool with these exact parameters: ${JSON.stringify(payload)}`);
}
...
export default function generateCommitMessageExtension(pi: ExtensionAPI) {
	...
	pi.registerCommand(COMMAND_NAME, command);
	pi.registerCommand(COMMAND_ALIAS, command);
}
