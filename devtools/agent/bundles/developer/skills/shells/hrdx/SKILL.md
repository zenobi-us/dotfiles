...
hrdx is a terminal multiplexer built for AI agent workflows — like tmux, but each pane knows if it holds a real PTY session for Codex CLI, Claude Code, pi, zot, a custom harness, or a plain shell. This repo manages hrdx config under `shells/hrdx/` and symlinks pieces of it into the real hrdx config directory via mise.
...
- This repo's managed files: `shells/hrdx/themes/*.json`, `shells/hrdx/keys.json`, and `shells/hrdx/harness.json`, each symlinked from `mise.unix.toml` (`~/.config/hrdx/...`) and `mise.windows.toml` (`%AppData%\hrdx\...`). Examples in this repo: `shells/hrdx/themes/rose-pine-moon.json`, `shells/hrdx/harness.json` (registers `lazygit` and `nvim` as quick-launch harnesses, no `busy` detection needed for either).
- `sounds.json` is managed by mise. It is symlinked from `shells/hrdx/sounds.json` in `mise.unix.toml` and `mise.windows.toml`.
